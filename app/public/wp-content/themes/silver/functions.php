...
function silver_scripts() {
	wp_enqueue_style( 'silver-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_enqueue_style( 'silver	-main', get_template_directory_uri().'./css/main.css',array(), _S_VERSION  );
	wp_style_add_data( 'silver-style', 'rtl', 'replace' );

	wp_enqueue_script( 'silver-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
function enqueue_custom_fonts (){
	if(!is_admin()){
		wp_register_style('raleWay', 'https://fonts.googleapis.com/css2?family=Raleway&display=swap');
		wp_register_style('roboto', 'https://fonts.googleapis.com/css2?family=Raleway&family=Roboto+Slab:wght@300&display=swap');
	}
}
